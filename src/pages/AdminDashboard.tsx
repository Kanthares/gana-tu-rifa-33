...
const AdminDashboard = () => {
  ...
  const [activeView, setActiveView] = useState<"none" | "create" | "delete" | "status">("none");
  ...
  // Función para obtener tickets desde el servidor
  ...
  // Obtener la lista de eventos y tickets al montar el componente
  useEffect(() => {
    fetchEvents();
    fetchTickets();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (eventData.id) {
      // PUT request para actualizar el evento
      try {
        const response = await fetch("https://ganaturifa.com/api/controller/Evento.php", {
          method: "PUT",
      ...
      // POST request para crear un nuevo evento
  ...
  // Maneja el cambio de estado de los tickets
  const handleStatusChange = async (ticketId: number, eventId: number, newStatus: string) => {
    try {
      const response = await fetch("https://ganaturifa.com/api/controller/Tickets.php", {
        ...
        body: JSON.stringify({ id: ticketId, event: eventId, status: newStatus }),
      });
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white/5 backdrop-blur-lg rounded-xl p-6 mb-6">
            ...
            <div className="flex gap-4 mb-4">
              ...
              <Button
                onClick={() => setActiveView(activeView === "delete" ? "none" : "delete")}
                variant="destructive"
              >
                Delete Event
              </Button>
              <Button
                onClick={() => setActiveView(activeView === "status" ? "none" : "status")}
                variant="outline"
              >
                Ticket Status
              </Button>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEventData({ ...eventData, startNumber: e.target.value })}
                      className="bg-white/10 border-white/20"
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setEventData({ ...eventData, quantity: e.target.value })}
                      min="0"
                      className="bg-white/10 border-white/20"
